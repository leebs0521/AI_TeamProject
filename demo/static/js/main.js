...
function greet() {
    setTimeout(function () {
        return sendMessage("강원대생을 위한 챗봇 알려두리입니다.", 'left');
    }, 1000);
...
function setUserName(username) {
    ...
    if (username != null && username.replace(" ", "" !== "")) {
        ...
        setTimeout(function () {
            return sendMessage("궁금한 내용을 알려주세요.", 'left');
        }, 2000);
...
function onSendButtonClicked() {
    ...
    if (userName == null) {
        ...
        if (messageText.includes('안녕')) {
            setTimeout(function () {
                return sendMessage("안녕하세요. 강원대 정보 알리미 알려두리 입니다.", 'left');
            }, 1000);
        ...
        } else if (messageText.includes('개발자')){
            setTimeout(function () {
                return sendMessage("알려두리는 파린이들이 만들었어요 :D", 'left');
            }, 1000);
